...
std::vector<Helpers::Mesh> Model::LoadMesh(const std::string argModelPath)
{
	Helpers::ModelLoader mesh;

	if (!mesh.LoadFromFile(argModelPath))
	{
		std::cerr << "ERROR: Couldn't load Model from file path, path = " << argModelPath << std::endl;
		return std::vector<Helpers::Mesh>();
	}

	return mesh.GetMeshVector();
}
