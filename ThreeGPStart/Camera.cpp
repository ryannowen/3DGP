...
namespace Helpers
{
	...
	void Camera::Initialise(const glm::vec3& startPos, const glm::vec3& startRots, float translationPerSecond, float rotationPerSecond)
	{
		...
		std::cout << "Q and E - moves the camera up and down the Y axis." << std::endl;
		...
		std::cout << "Holding left CTRL key accelerates movement" << std::endl << std::endl;
	}
	...
	void Camera::Update(GLFWwindow* window, float timePassedSecs)
	{
		...
		// Mouse Input
		{
			...
			else if (state == GLFW_PRESS)
			{
				...
				float pitchChange = ((float)xpos - initialPos.x) * 0.001f * 5.0f/** radiansPerSecond*/;
				float yawChange = ((float)ypos - initialPos.y) * 0.001f * 5.0f /** radiansPerSecond*/;
